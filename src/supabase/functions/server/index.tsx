...
async function generateNewPrompt(groupId: string, group: any, creationDate?: string) {
    ...
    const availableAtDepth = getPromptsAtDepthRange(group.depthLevel, 2).length;
...
function getPromptsAtDepthRange(targetDepth: number, range: number): Prompt[] {
...
function selectSmartPrompt(
  ...
  // Every 8th prompt, push one level deeper (if not at max)
  const shouldPushDeeper = (promptCount + 1) % 8 === 0;
  const targetDepth = shouldPushDeeper ? Math.min(groupDepth + 1, 10) : groupDepth;
  
  // Get prompts within depth range (±2 levels for flexibility)
  let candidatePrompts = getPromptsAtDepthRange(targetDepth, 2)
